...
<head>
    ...
    <style>
        ...
        .msg-warning { color: orange; margin-bottom: 12px; }
        table { border-collapse: collapse; width: 100%; max-width: 700px; }
        th, td { border: 1px solid #ccc; padding: 8px; text-align: left; }
        th { background: #f0f0f0; }
        .qty-form { display: flex; align-items: center; gap: 6px; }
        .qty-form input[type="number"] { width: 60px; padding: 3px; border: 1px solid #ccc;
                                          border-radius: 3px; }
        .btn { display: inline-block; padding: 4px 10px; text-decoration: none;
               border: 1px solid #999; border-radius: 3px; cursor: pointer; font-size: 13px; }
...
<% if (stockMessage != null) { %>
    <p class="msg-warning"><%= stockMessage %></p>
<% } %>

<% if (cart == null || cart.isEmpty()) { %>
    <p class="empty-msg">Your cart is empty.</p>
    <a href="${pageContext.request.contextPath}/products" class="btn btn-shop">Browse Products</a>
<% } else { %>
    <table>
        <thead>
            <tr>
